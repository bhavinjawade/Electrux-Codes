...
public:
	...
	std::vector<std::string> Execute(Electrux::SlicedTime &time)
	{
		...
		if (!jobs.empty())
		{
			for (auto it = jobs.begin(); it != jobs.end(); ++it)
			{
				if (it->exec_time.getDateTime(Electrux::SlicedTime::Data::HOURS) == time.getDateTime(Electrux::SlicedTime::Data::HOURS) && it->exec_time.getDateTime(Electrux::SlicedTime::Data::MINUTES) == time.getDateTime(Electrux::SlicedTime::Data::MINUTES))
				{
